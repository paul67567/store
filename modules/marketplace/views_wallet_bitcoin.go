...
func (c *Context) BitcoinWalletSendStep2GET(w web.ResponseWriter, r *web.Request) {
	...
	if withdraw {
		c.Amount = c.UserBitcoinBalance.Balance - c.BTCFee
		if c.Amount < 0 {
			c.Error = "Amount is smaller than transaction fee"
		}
	} else if c.UserBitcoinBalance.Balance < (c.Amount + c.BTCFee) {
...
func (c *Context) BitcoinWalletActions(w web.ResponseWriter, r *web.Request) {
	...
	util.RenderTemplateOrAPIResponse(w, r, "wallet/bitcoin/actions", c, c.IsAPIRequest)
}
