...
type Message struct {
	...
	ReportedByUserUuid string     `json:"reported_by_user_uuid" gorm:"index"`
	ReviewedByUserUuid string     `json:"reviewed_by_user_uuid" gorm:"index"`
...
func setupThreadsViews() {
	...
	database.Exec("DROP MATERIALIZED VIEW IF EXISTS vm_thread_counts CASCADE;")
	database.Exec(`
		CREATE MATERIALIZED VIEW vm_thread_counts AS (
			select 
				* 
			FROM 
				v_thread_counts
	);`)
	database.Exec("create index on vm_thread_counts (parent_uuid);")

	database.Exec("DROP MATERIALIZED VIEW IF EXISTS vm_threads CASCADE;")
	database.Exec(`
		CREATE MATERIALIZED VIEW vm_threads AS (
			SELECT 
				* 
			FROM 
				v_threads
	);`)
	database.Exec("create index on vm_threads (type);")
	database.Exec("create index on vm_threads (uuid);")
	database.Exec("create index on vm_threads (sender_user_uuid);")
	database.Exec("create index on vm_threads (reciever_user_uuid);")
}

func RefreshThreadCountMaterializedView() {
	database.Exec("REFRESH MATERIALIZED VIEW vm_thread_counts;")
}

func RefreshViewThreadsMaterializedView() {
	database.Exec("REFRESH MATERIALIZED VIEW vm_threads;")
}
