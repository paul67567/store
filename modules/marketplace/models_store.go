...
type Store struct {
	...
	Storename string `json:"storename" gorm:"type:varchar(16);unique;index"`
...
func (u Store) Save() error {
	...
	cacheClearStoreByStorename(u.Storename)
...
func (s Store) CalculateScore() float64 {
	score := float64(0.0)
	scoreCounter := 0
	for _, review := range s.RatingReviews {

		currentTransactionStatus := review.Transaction.CurrentPaymentStatus()
		if currentTransactionStatus == "FAILED" || currentTransactionStatus == "CANCELLED" {
			continue
		}

		score += float64(review.SellerScore)
		scoreCounter += 1
	}
	if scoreCounter > 0 {
		score /= float64(scoreCounter)
	}
...
type ViewStore struct {
	*Store
	...
	HasSignedVendorshipAgreement bool `json:"has_signed_vendorship_agreement"`
	ActivatedAccount             bool `json:"activated_account"`
...
func (s Store) ViewStore(lang string) ViewStore {
	viewStore := ViewStore{
		...
		VerificationRequired:         !(s.PGP != "" && s.ActivatedAccount() && s.HasSignedVendorshipAgreement()),
	}
	return viewStore
}
...
func CacheFindStoreByUserUuid(userUuid string) (*Store, error) {
	var (
		store *Store
		err   error
	)

	key := "store-by-user-uuid-" + userUuid

	if cStore, err := cache15m.Get(key); err != nil && err.Error() == "Key not found." {
		store, _ = FindStoreByUserUuid(userUuid)
		cache15m.Set(key, store)
	} else {
		if cStore != nil {
			store = cStore.(*Store)
		}
...
func cacheClearStoreByUserUuid(userUuid string) {
	key := "store-by-user-uuid-" + userUuid
	cache15m.Remove(key)
}

func cacheClearStoreByStorename(storename string) {
	key := "store-by-storename-" + storename
	cache15m.Remove(key)
}

func CacheFindStoreByStorename(storename string) (*Store, error) {
	var (
		store *Store
		err   error
	)
	...
	if cStore, err := cache15m.Get(key); err != nil && err.Error() == "Key not found." {
		store, _ = FindStoreByStorename(storename)
		cache15m.Set(key, store)
	} else {
		if cStore != nil {
			store = cStore.(*Store)
