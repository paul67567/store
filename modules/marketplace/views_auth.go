...
func (c *Context) ViewVerificationPlanPurchasePOST(w web.ResponseWriter, r *web.Request) {
	...
	case "btc":
		...
		receipt, err := userWallets.Send(address, c.PriceBTC)
		if err != nil {
			c.Error = err.Error()
			c.ViewVerificationPlanPurchaseGET(w, r)
			return
		...
		btcPaymentResult, err := receipt.BTCPaymentResult()
		if err != nil {
			c.Error = err.Error()
			c.ViewVerificationPlanPurchaseGET(w, r)
			return
		...
		if btcPaymentResult.Hash == "" {
			c.Error = "Unable to pay for account"
			c.ViewVerificationPlanPurchaseGET(w, r)
			return
		}

		rawTx = btcPaymentResult.RawTransaction
	...
	EventUserSubscription(*c.ViewUser.User, c.Account, c.Currency, address, rawTx)
	util.RedirectOrAPIResponse(w, r, "/verification/plan", c, c.IsAPIRequest)
}
...
func (c *Context) ViewVerificationAgreementGET(w web.ResponseWriter, r *web.Request) {
	...
	util.RenderTemplateOrAPIResponse(w, r, "store/verification_agreement", c, c.IsAPIRequest)
}
...
func (c *Context) ViewVerificationAgreementPOST(w web.ResponseWriter, r *web.Request) {
	...
	isSignatureCorrect, err := util.CheckSignature(signedAgreement, agreement, c.ViewUserStore.PGP)
...
func (c *Context) Login1FactorPOST(w web.ResponseWriter, r *web.Request) {
	isCaptchaValid := base64Captcha.VerifyCaptcha(r.FormValue("captcha_id"), r.FormValue("ca"))
...
func (c *Context) ViewUserSettingsGET(w web.ResponseWriter, r *web.Request) {
	...
	c.APISession, _ = FindOrCreateAPISession(*c.ViewUser.User)

	util.RenderTemplateOrAPIResponse(w, r, "auth/settings-user", c, c.IsAPIRequest)
}
...
func (c *Context) ViewStoreSettingsGET(w web.ResponseWriter, r *web.Request) {
	...
	util.RenderTemplateOrAPIResponse(w, r, "auth/settings-store", c, c.IsAPIRequest)
}
...
func (c *Context) SetupPGPViaDecryptionStep2POST(w web.ResponseWriter, r *web.Request) {
	...
	if decryptedText == secretText {
		...
		http.Redirect(w, r.Request, "/settings/user?section=system", 302)
		return
	} else {
