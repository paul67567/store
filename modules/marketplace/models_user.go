...
type User struct {
	...
	BitcoinMultisigPublicKey string `json:"bitcoin_public_key"`
	...
	// Misc settings
	TwoFactorAuthentication bool `json:"2fa" gorm:"index"`
...
func GetExtendedUsersPage(page, pageSize int, orderBy, query string) []ExtendedUser {
	...
	if orderBy == "last_login" {
		...
		orderBy = "bitcoin_balance desc nulls last, ethereum_balance desc nulls last"
	} else {
...
func FindUserByUuid(uuid string) (*User, error) {
	...
	// return cacheFindUserByUuid(uuid)
}
...
func FindUserByUsername(username string) (*User, error) {
	...
	// return cacheFindUserByUsername(username)
}
...
type ViewUser struct {
	*User
	...
	// Wallets
	BitcoinBalance        apis.BTCWalletBalance      `json:"bitcoin_balance"` // Bitcoin Wallets
	BitcoinWallets        UserBitcoinWallets         `json:"-"`
	BitcoinWallet         UserBitcoinWallet          `json:"bitcoin_wallet"`
	BitcoinWalletActions  []UserBitcoinWalletAction  `json:"-"`
	EthereumBalance       apis.ETHWalletBalance      `json:"ethereum_balance"` // Ethereum Wallets
	EthereumWallets       UserEthereumWallets        `json:"-"`
	EthereumWallet        UserEthereumWallet         `json:"ethereum_wallet"`
...
func (s User) ViewUser(lang string, excludeStore bool) ViewUser {
	viewUser := ViewUser{
		User:                &s,
...
func (users Users) ViewUsers(lang string) []ViewUser {
	viewUsers := []ViewUser{}
	for _, user := range users {
		viewUsers = append(viewUsers, user.ViewUser(lang, false))
	}
	return viewUsers
}
