...
type SupportTicket struct {
	...
	ThreadUuid      string `json:"-" gorm:"index"`
	UserUuid        string `json:"-" gorm:"index"`
	TransactionUuid string `json:"transction_uuid" gorm:"index"`
	...
	SupportTicketStatuses []SupportTicketStatus `json:"statuses"`
}
...
type SupportTicketStatus struct {
	...
	UserUuid          string `json:"-" gorm:"index"`
	SupportTicketUuid string `json:"-" gorm:"index"`
...
func CreateSupportTicket(
	...
	if transactionUuid != "" && transactionUuid != "None" {
		_, err := FindTransactionByUuid(transactionUuid)
		if err != nil {
			return nil, err
		}
	}
...
type ViewSupportTicket struct {
	*SupportTicket
	ViewUser      ViewUser `json:"user"`
	CurrentStatus string   `json:"current_status"`
	CreatedAtStr  string   `json:"create_at_str"`
...
func (st SupportTicket) ViewSupportTicket(lang string) ViewSupportTicket {
	...
	return ViewSupportTicket{
		SupportTicket: &st,
		ViewUser:      st.User.ViewUser(lang, false),
