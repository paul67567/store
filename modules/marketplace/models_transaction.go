...
var (
	...
	stuckDuration, _     = time.ParseDuration(MARKETPLACE_SETTINGS.StuckDuration)
)
...
type Transaction struct {
	...
	MobileAppTransaction bool   `json:"mobile_app_transaction"`
	...
	Buyer          User                `json:"buyer"  gorm:"foreignkey:buyer_uuid"`
...
func (t *Transaction) RefreshTransactionStatus() {
	t.Status = FindTransactionStatusesForTransaction(t.Uuid)
}

func (t Transaction) StatusSortedByTime() []TransactionStatus {
	// status := t.GetCurrentTransactionStatuses()
...
func (t Transaction) PartialRefund(comment, userUuid string, refundPercent float64) error {

	switch t.Type {
	case "bitcoin":
		return t.BitcoinTransaction.PartialRefund(t, comment, userUuid, refundPercent)
	case "ethereum":
		return t.EthereumTransaction.PartialRefund(t, comment, userUuid, refundPercent)
	}
...
func (t Transaction) FundFromUserWallets(user User) (PaymentReceipt, error) {
	...
	if err == nil {
		return receipt, t.SetTransactionStatus(
			"PENDING",
			amount*0,
			fmt.Sprintf("Transaction funded from user wallet, amount: %f, fee: %f", amount, fee),
			user.Uuid,
			&receipt,
		)
	}
...
func CreateTransactionForPackage(
	...
	var (
		...
		transaction   = Transaction{
			...
			Status:               status,
			MobileAppTransaction: mobilePurchase,
		}
	)
...
func FindTransactionByUuidRaw(uuid string) (*Transaction, error) {
	var item Transaction
	err := database.Where(&Transaction{Uuid: uuid}).First(&item).Error
	if err != nil {
		return nil, err
	}
	return &item, err
...
func FindTransactionStatusesForTransaction(uuid string) []TransactionStatus {
	var statuses []TransactionStatus

	database.Where("transaction_uuid=?", uuid).Find(&statuses)
...
func FindCompletedAndNotDispatchedTransactions() []Transaction {
	var uuids []string
	database.
		Table("v_current_cummulative_transaction_statuses").
		Where("current_status IN ('COMPLETED') and current_shipping_status IN ('DISPATCH PENDING') and (updated_at - created_at)  >= interval '72 hours'").
		Order("created_at ASC").
		Pluck("uuid", &uuids)
	return loadTransactionsByUuid(uuids)
}
...
func FindOldReleasedTransactions() []Transaction {
	return findTransactionsWithCurrentStatusAndTimestamp(
		"RELEASED",
		time.Now().Add(-completedDuration),
	)
}

func FindOldCompletedTransactions() []Transaction {
	return findTransactionsWithCurrentStatusAndCreatedTimestamp(
		"COMPLETED",
		time.Now().Add(-completedDuration),
	)
}
...
func FindOldPendingTransactions() []Transaction {
	return findTransactionsWithCurrentStatusAndTimestamp(
		"PENDING",
		time.Now().Add(-pendingDuration),
	)
}

func FindStuckCompletedTransactions() []Transaction {
	return findTransactionsWithCurrentStatusAndTimestamp(
		"COMPLETED",
		time.Now().Add(-stuckDuration),
	)
}
