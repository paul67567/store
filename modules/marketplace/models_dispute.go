...
type Dispute struct {
	...
	ResolverUserUuid string `json:"-" sql:"index"`
...
func (dispute Dispute) PartialRefund(userUuid string, percent float64) error {
	dispute.Status = "PARTIL REFUND"
	...
	return tx.PartialRefund("Partial refund", userUuid, percent)
}
...
func FindDisputeClaimByID(disputeId uint) (*DisputeClaim, error) {
	...
	err := database.
		First(&disputeClaim, "dispute_uuid = ?", disputeId).
		Error
