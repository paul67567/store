...
func (bt BitcoinTransaction) PartialRefund(t Transaction, comment, userUuid string, refundPercent float64) error {
	var (
		err         error
		addressFrom = t.Uuid
	...
	storeAddress := store.BitcoinAddress()
	buyerAddress := buyer.BitcoinAddress()
	resolverAddress := resolver.BitcoinAddress()
...
func (bt BitcoinTransaction) Release(t Transaction, comment, userUuid string) error {
	...
	if vendorInviter != nil && !t.MobileAppTransaction {
		inviterPercent := MARKETPLACE_SETTINGS.FreeAccountReferralPercent
